...
export default function Auth() {
  ...
  // Handle redirect callback from Google OAuth if redirected
  useEffect(() => {
  ...
  const handleTriggerGoogle = () => {
    ...
    // 1. Trigger Google's official OAuth 2.0 popup (opens Google.com with device Gmail accounts)
    if (window.google?.accounts?.oauth2) {
      try {
        ...
        console.warn('OAuth2 popup error, falling back to direct Google redirect:', e);
      }
    }

    // 2. Direct redirect to official accounts.google.com login page
  ...
  const handleTriggerApple = () => {
    setError('Apple Sign-In is configured for Apple devices and Safari browsers.');
  };
