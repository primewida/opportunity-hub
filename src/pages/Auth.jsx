...
export default function Auth() {
  ...
  // Initialize official Google Identity Services & Render native Google Button
  ...
  // Handle redirect callback from Google OAuth if redirected via URL
  useEffect(() => {
  ...
  const handleTriggerGoogle = () => {
    ...
    // 1. If Google Identity Services rendered button exists, trigger its native click
    const renderedBtn = googleBtnRef.current?.querySelector('div[role=button], button, iframe');
    if (renderedBtn) {
      try {
        renderedBtn.click();
        return;
    ...
    // 2. Direct browser navigation to Google's official accounts.google.com chooser
    const redirectUri = window.location.origin + '/auth';
    const googleAuthUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${encodeURIComponent(GOOGLE_CLIENT_ID)}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=token&scope=${encodeURIComponent('openid email profile')}&prompt=select_account`;
  ...
  return (
    <div className="auth">
      <div className="auth__card animate-scaleIn">
        ...
        {/* Native Official Google Sign In Container */}
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 'var(--space-md)', width: '100%', minHeight: '44px' }}>
